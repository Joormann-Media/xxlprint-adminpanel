...
public function bulkAssignAddresses(
    ...
    #[Route('/api/{id<\d+>}', name: 'app_school_api', methods: ['GET'])]
    public function api(School $school, SchoolkidsRepository $kidsRepo): JsonResponse
    {
        $kids = $kidsRepo->findBy(['school' => $school]);
        ...
        return $this->json([
            'school' => [
                ...
                'address' => trim($school->getStreet().' '.$school->getStreetNo().', '.$school->getZip().' '.$school->getCity()),
            ],
            'kids' => array_map(fn($k) => [
                'id'            => $k->getId(),
                ...
                'lat'           => $k->getLatitude(),
                'lng'           => $k->getLongitude(),
                'address'       => trim($k->getStreet().' '.$k->getStreetNumber().', '.$k->getZip().' '.$k->getCity()),
