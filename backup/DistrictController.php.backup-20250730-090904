...
final class DistrictController extends AbstractController
{
    ...
    #[Route('/{id<\d+>}', name: 'app_district_show', methods: ['GET'])]
    ...
    #[Route('/{id<\d+>}/edit', name: 'app_district_edit', methods: ['GET', 'POST'])]
    ...
    #[Route('/{id<\d+>}', name: 'app_district_delete', methods: ['POST'])]
    public function delete(Request $request, District $district, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid('delete'.$district->getId(), $request->getPayload()->getString('_token'))) {
